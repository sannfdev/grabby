...
df | awk -v c1="$c1" -v c2="$c2" -v c3="$c3" '/^\/dev\// {barc=int($5 * 30 / 100); printf("│%s %11s%s │ ", c3, "- "substr($1, 6), c1); for(i=0;i<=barc;i++) printf("%s━",c2);for(i=barc;i<=30;i++) printf("%s-", c1); printf(" %s%s%s%%\n", c2, $5-0, c1)}'
printf '╰─────────────╯\033[0m\n'